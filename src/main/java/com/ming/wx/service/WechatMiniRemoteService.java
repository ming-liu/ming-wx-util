...
import com.ming.wx.bean.MiniUserAccessTokenBean;
import com.ming.wx.bean.TicketBean;
import com.ming.wx.bean.UserInfoBean;

public class WechatMiniRemoteService {

	private static Logger logger = Logger.getLogger(WechatMiniRemoteService.class);

	private static final String TokenUrl = "https://api.weixin.qq.com/cgi-bin/token";
	private static final String OauthTokenUrl = "https://api.weixin.qq.com/sns/jscode2session";
	...
	public MiniUserAccessTokenBean getOauthAccessToken(String appId, String appSecret, String code) {
		try {
			URI uri = new URIBuilder(OauthTokenUrl).setParameter("appid", appId).setParameter("secret", appSecret).setParameter("js_code", code)
			...
			MiniUserAccessTokenBean token = JsonUtil.fromJson(string, MiniUserAccessTokenBean.class);
			return token;
		} catch (Exception e) {
			logger.error(e, e);
		}
